...
def balanced_split(annos: Dict[str, Dict[str, List["BBox"]]], train_ratio: float) -> Tuple[List[str], List[str]]:
    """balanced_split. Return train and val image names

    :param annos: Dict[str, Dict[str, List["BBox"]]]
    :param train_ratio: float
    :return: Tuple[List[str], List[str]]
    """
    obj_imgs = defaultdict(list)
    for img, annos in annos.items():
        for obj in annos.keys():
            obj_imgs[obj].append(img)
...
def read_yolo_labels(yolo_label_path, yolo_data_path,
                     cat_mapper: Dict[str, str] = None,
                     conf_threshold=0.5,
                     extra_conf_thres=None):
    ...
    for label_path in tqdm(label_dir.glob('*.txt')):
        ...
        with open(label_path, 'r') as f:
            ...
            for line in lines:
                ...
                if len(items) == 6:
                    conf = items[5]
                thres = conf_threshold
                if extra_conf_thres is not None:
                    thres = extra_conf_thres.get(id2class[int(class_id)], conf_threshold)
                if conf < thres:
                    continue
                x_min = x - w / 2
                y_min = y - h / 2
...
def read_voc_labels(gt_label_path: str) -> Dict[str, Dict[str, List[BBox]]]:
