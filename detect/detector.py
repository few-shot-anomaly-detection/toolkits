...
# @detectors.register('sleeper_crack')
class FasteningOppositeDetector(Detector):
    def detect(self, ps: PositionStructure, img_name: str) -> List[BBox]:
        ...
        for crack in cracks:
            ...
            if crack.parent is not None and crack.parent.cat == 'sleeper':
                ret.append(crack.bbox)
        return ret
...
class FasteningLackDetector(Detector):
    def detect(self, ps: PositionStructure, img_name: str) -> List[BBox]:
        ...
        for spike in spikes:
            ...
            missed = True
            region = BBox(
                0 if spike_x > rail_center[0] else rail_center[0],
                spike_y - height,
                rail_center[0] if spike_x > rail_center[0] else 1,
                spike_y + height
            )
        ...
        def check(comps):
            for cat in ['bolt_in', 'component', 'opening', 'a', 'b', 'plate']:
        ...
        for sleeper in sleepers:
            ...
            if check(left_comps) or check(right_comps):
                ret.append(1)
        return ret
