...
def main(args):
    ...
    fp = dict()
    fn = dict()
    for img, (gt, pred) in merged_labels.items():
        ...
        for cat in detectors.keys():
            ...
            anomaly_pred = detector.detect(ps, img)
            if anomaly_gt and anomaly_pred:
                tp[cat] += 1
                logger.debug('detect %s tp %s', cat, img)
            if anomaly_gt and not anomaly_pred:
                fn[cat] += 1
                logger.debug('detect %s fn %s', cat, img)
                if args.save:
                    img_path = labeled_preds_dir / f'{img}.jpg'
                    if not img_path.exists():
                        img_path = labeled_preds_dir / f'{img}.png'
                    img = cv2.imread(str(img_path))
                    img = draw_bboxes(img, anomaly_gt)
                    cv2.imwrite(str(save_paths[cat] / 'fn' / img_path.name), img)
            if not anomaly_gt and anomaly_pred:
                fp[cat] += 1
                logger.debug('detect %s fp %s', cat, img)
                if args.save:
                    img_path = labeled_preds_dir / f'{img}.jpg'
                    if not img_path.exists():
                        img_path = labeled_preds_dir / f'{img}.png'
